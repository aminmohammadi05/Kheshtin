...
export class AppService {
  ...
  public addToFavorites(product: Property, direction) {
    if (!this.Data.favorites.filter(item => item.id === product.id)[0]) {
      this.Data.favorites.push(product);
      this.snackBar.open('The product "' + product.title + '" has been added to favorites.', '×', {
