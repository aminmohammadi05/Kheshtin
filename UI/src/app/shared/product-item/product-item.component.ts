...
export class ProductItemComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  onMoodBoard() {
    // this.store.pipe(select(getAllUserMoodBoardCandidateProducts),
    // map(products => {
    //   return products.filter(item => item.productId === this.product.productId)[0] === null ||
    //   products.filter(item => item.productId === this.product.productId)[0] === undefined;
    // }));
  }
  ...
  public getCategoriesNames() {
    if (this.product) {
      // return this.store.pipe(select(getCategoryListById(this.product.productCategoryList.filter(x => x.display).map(x => x.categoryId))),
      // map((cats) => {
      //   if (cats && cats[0]) {
