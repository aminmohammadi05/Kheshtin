...
getCategoriesAuth(userId: any): Observable<Category[]> {
  return this.http.get<Category[]>(this.baseUrl + `users/${userId}/categories/GetCategories/`);
}

getProductsOfCategory(id: { toString: () => string; }, term = 'filter=', pageNumber = 0, pageSize = 3): Observable<PaginatedResult<Product[]>> {
